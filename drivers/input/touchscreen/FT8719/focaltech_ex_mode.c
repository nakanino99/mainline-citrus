...
static int fts_ex_mode_switch(enum _ex_mode mode, u8 value)
{
	...
	case MODE_CHARGER:
		...
		break;
	case MODE_HEADSET:
		ret = fts_write_reg(FTS_REG_HEADSET_MODE_EN, m_val);
		if (ret < 0) {
			FTS_ERROR("MODE_HEADSET switch to %d fail", m_val);
		}
		break;
	default:
...
static ssize_t fts_headset_mode_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	u8 val = 0;
	...
	mutex_lock(&input_dev->mutex);
	fts_read_reg(FTS_REG_HEADSET_MODE_EN, &val);
	count = snprintf(buf + count, PAGE_SIZE, "HEADSETMode:%s\n",
					 ts_data->headset_mode ? "On" : "Off");
	count += snprintf(buf + count, PAGE_SIZE, "HEADSET Reg(8C):%d\n", val);
	mutex_unlock(&input_dev->mutex);

	return count;
}

static ssize_t fts_headset_mode_store(
	struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		if (!ts_data->headset_mode) {
			FTS_DEBUG("enter headset mode");
			pr_err("enter headset mode");
			ret = fts_ex_mode_switch(MODE_HEADSET, ENABLE);
			if (ret >= 0) {
		...
		if (ts_data->headset_mode) {
			FTS_DEBUG("exit headset mode");
			pr_err("exit headset mode");
			ret = fts_ex_mode_switch(MODE_HEADSET, DISABLE);
			if (ret >= 0) {
				ts_data->headset_mode = DISABLE;
			}
		}
	}

	FTS_DEBUG("headset  mode:%d", ts_data->headset_mode);
	return count;
}
...
static DEVICE_ATTR(fts_headset_mode, S_IRUGO | S_IWUSR,
				   fts_headset_mode_show, fts_headset_mode_store);

static struct attribute *fts_touch_mode_attrs[] = {
	&dev_attr_fts_glove_mode.attr,
	&dev_attr_fts_cover_mode.attr,
	&dev_attr_fts_charger_mode.attr,
	&dev_attr_fts_headset_mode.attr,
	NULL,
};
...
int fts_ex_mode_recovery(struct fts_ts_data *ts_data)
{
	...
	if (ts_data->headset_mode) {
		fts_ex_mode_switch(MODE_HEADSET, ENABLE);
	}
	return 0;
}
