...
// extern int hq_regiser_hw_info(enum hardware_id id, char *device_name);  (removed: vendor-specific, not in mainline)
								 ...
								 unsigned long event, void *data)
{
	...
	case DRM_PANEL_BLANK_UNBLANK:
		if (DRM_PANEL_EARLY_EVENT_BLANK == event) {
			...
			FTS_DEBUG("resume: DRM_PANEL_BLANK_UNBLANK, DRM_PANEL_EVENT_BLANK\n");
			queue_work(fts_data->ts_workqueue, &fts_data->resume_work);
		}
		break;
	case DRM_PANEL_BLANK_POWERDOWN:
		if (DRM_PANEL_EARLY_EVENT_BLANK == event) {
			FTS_DEBUG("resume: DRM_PANEL_BLANK_POWERDOWN, DRM_PANEL_EARLY_EVENT_BLANK\n");
			cancel_work_sync(&fts_data->resume_work);
			fts_ts_suspend(ts_data->dev);
		} else if (DRM_PANEL_EVENT_BLANK == event) {
			FTS_INFO("suspend: event = %lu, not care\n", event);
...
// 	hq_regiser_hw_info(HWID_LCM, "incell,vendor:truly,IC:ft8719(focal)");
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	//strscpy(ts_data->fts_lockdowninfo, fts_lockdown, 17);
...
#if FTS_GESTURE_EN
		ret = fts_ts_enable_regulator(false);
   		if (ret < 0) {
       			FTS_ERROR("Failed to enable regulator");
   	 	}
#endif
...
#endif
#if 0
static int fill_ctrl_data(struct spi_device *client){
    struct spi_geni_qcom_ctrl_data *delay_params;
    struct device_node *dp = client->dev.of_node;
    u32 cs_clk_delay;
    int ret = 0;
//    int inter_words_delay;

    ret = of_property_read_u32(dp, "spi-cs-clk-delay", &cs_clk_delay);
    if(ret < 0){
        FTS_INFO("read spi-cs-clk-delay failed!(%d)\n", ret);
        return ret;
    }
    FTS_INFO("cs_clk_delay = %d\n", cs_clk_delay);
    delay_params = (struct spi_geni_qcom_ctrl_data *) devm_kzalloc(&client->dev,
            sizeof(struct spi_geni_qcom_ctrl_data),
            GFP_KERNEL);
    if (delay_params == NULL) {
        FTS_ERROR("failed to allocated memory for delay_params\n");
        return -ENOMEM;
    }
    delay_params->spi_cs_clk_delay = cs_clk_delay;
    client->controller_data = (void*) delay_params;
    return ret;
}
#endif
...
static int __init is_fts_lockdown_info_detect(char *str)
{
	strscpy(fts_lockdown, str, sizeof(fts_lockdown));
	return 0;
}
