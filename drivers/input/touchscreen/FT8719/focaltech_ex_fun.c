	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	count += snprintf(buf + count, PAGE_SIZE, "headset stat:0x%02x\n", val);
...
static const struct proc_ops proc_tp_lock_down_info_fops = {
	.proc_open = focal_tp_lock_down_info_open,
	.proc_read = seq_read,
};
